document.addEventListener('DOMContentLoaded', () => {
    ...
    logoutBtn.addEventListener('click', () => {
        ...
        localStorage.removeItem(TASK_STORAGE_KEY); // Also clear tasks on logout
        tasks = [];
        showLoggedOutView();
    });
    
    // --- APPLICATION LOGIC (Your existing code) ---
    // (I've copied all your task functions here)
    ...
    function renderApp() {
        ...
        const tasksToRender = tasks.filter(task => currentView === 'active' ? !task.completed : task.completed);
        ...
        taskItem.innerHTML = `
            <div class="task-header">
                ...
                <div class="task-actions">
                    <button class="edit-btn" aria-label="Edit task"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/></svg></button>
                    <button class="delete-btn" aria-label="Delete task"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg></button>
                </div>
            </div>
            <div class="task-body">
        ...
        return `
            ...
            <div class="task-notes ${task.isEditingNotes ? 'is-editing' : ''}">
                <div class="notes-display ${hasNotes ? '' : 'is-empty'}">
                    ...
                    <div class="notes-text">${hasNotes ? task.notes.replace(/\n/g, '<br>') : 'Add a description...'}</div>
                </div>
    ...
    // All your handler functions (handleFormSubmit, handleDeleteTask, etc.) go here
    async function handleFormSubmit(e) {
        ...
        const newTask = {
            id: `task-${Date.now()}`, text: taskText, completed: false, subtasks: [],
            isGenerating: true, isOpen: true, notes: '', isEditingNotes: false,
        };
        ...
        try {
            ...
            if (taskToUpdate) {
                taskToUpdate.subtasks = (generatedSubtasks || []).map((text, i) => ({
                    id: `${newTask.id}-subtask-${Date.now()}-${i}`, text, completed: false
                }));
            }
        } catch (error) {
    ...
    function handleToggleAccordion(taskId) {
        ...
        if (task) { task.isOpen = !task.isOpen; saveTasks(); renderApp(); }
    }
    function handleEnterNotesEditMode(taskId) {
        const task = tasks.find(t => t.id === taskId);
        if (task) { task.isEditingNotes = true; renderApp(); }
    }
    ...
    function handleCancelNotesEdit(taskId) {
        const task = tasks.find(t => t.id === taskId);
        if (task) { task.isEditingNotes = false; renderApp(); }
    }
    ...
    function handleToggleTask(taskId) {
        ...
        if (task) {
            ...
            if (task.completed) { task.subtasks.forEach(subtask => subtask.completed = true); }
            saveTasks();
            renderApp();
        }
    }
    ...
    function handleDeleteSubtask(taskId, subtaskId) {
        const task = tasks.find(t => t.id === taskId);
        if (task) { task.subtasks = task.subtasks.filter(st => st.id !== subtaskId); saveTasks(); renderApp(); }
    }
    function handleEditTask(editBtn, taskId) {
        ...
        if (isEditing) {
            ...
            editInput.type = 'text';
            editInput.value = h2.textContent || '';
            editInput.className = 'edit-input main-task-edit';
            headerMain.replaceChild(editInput, h2);
            editInput.focus();
            editInput.select();
            const saveOnKey = (e) => { if (e.key === 'Enter' || e.key === 'Escape') { e.preventDefault(); editBtn.click(); } };
    ...
    // This is the first thing that runs. It checks if the user is logged in.
    handleAuthState();
});